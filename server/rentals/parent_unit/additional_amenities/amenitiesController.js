...
const getAmenities = (req, res) => {
  pool.query(
    ...
    (error, results) => {
      if (results.rows < 1) {
        res.status(404).send("No additional amenities found");
      } else if (error) {
        throw error;
...
const editAmenity = (req, res) => {
  pool.query(queries.getAmenityById, [req.params.id], (error, results) => {
    if (error) throw error;
    if (results.rows.length > 0) {
      pool.query(
        ...
        (error, results) => {
          if (error) throw error;
          res.status(200).send("Successfully changed amenity");
        }
      );
    } else res.status(404).send("Amenity not found in the database.");
  });
};
...
const deleteAmenity = (req, res) => {
  pool.query(queries.deleteAmenity, [req.params.id], (error, results) => {
    if (error) throw error;
    res.status(200).send("Successfully removed amenity");
  });
};
